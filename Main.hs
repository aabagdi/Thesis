...
updateMario (PlaySound [url, url2]) m = batchEff m
#ifdef __GHCJS__
  --Prelude.map ((NoOp <$) . play) [url, url2]
  [play url >> pure NoOp, threadDelay 1000000 >> pure NoOp, play url2 >> pure NoOp]
#else
  pure NoOp
#endif
updateMario (GetArrows arrs) m = newModel <# do
    pure $
      ...
      then (PlaySound ["https://github.com/aabagdi/Chord-Helper/blob/8d9ca23dd97dba6debcc66b480ff0bf091e01fc0/Chord%20Helper/Guitar/AMajor.wav?raw=true", "https://github.com/aabagdi/Chord-Helper/blob/8d9ca23dd97dba6debcc66b480ff0bf091e01fc0/Chord%20Helper/Guitar/GMajor.wav?raw=true"])
      else NoOp
  where
...
marioStyle Model {..} gy =
  M.fromList [ ("transform", matrix dir x $ abs (y + gy) )
             ...
             , ("background-image", "url(https://mario.haskell-miso.org/imgs/mario.png)")
